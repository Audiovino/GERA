...
const Header: React.FC<HeaderProps> = ({ scrollToRef }) => {
    ...
    return (
        <>
            <header className="sticky top-0 bg-gray-900 border-b border-gray-700 z-50">
                <div className="container mx-auto px-4 md:px-8 flex justify-between items-center py-4">
                    ...
                    <nav className="hidden lg:flex space-x-4 xl:space-x-6 items-center">
                        {navItems.map((item) => (
                            <button key={item.name} onClick={() => scrollToRef(item.ref)} className="text-gray-300 hover:text-blue-400 transition-colors text-sm xl:text-base whitespace-nowrap">
                                {item.name}
                            </button>
                        ))}
                    </nav>
            ...
            {/* Mobile Menu */}
            <div className={`fixed inset-0 z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
