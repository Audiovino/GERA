...
const Hero: React.FC<HeroProps> = ({ scrollToContact, scrollToSell, openServicesModal }) => {
    ...
    return (
        <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 min-h-[calc(100vh-80px)] text-white flex items-center justify-center text-center px-4 py-20">
            <div className="max-w-4xl flex flex-col items-center">
                <img
src="/perfil_v2.png"
alt="Leiserson Gerardo - Asesor Inmobiliario SOLDATI"
className="w-64 h-64 md:w-80 md:h-80 rounded-2xl object-contain bg-gray-800/50 border-4 border-blue-500/30 shadow-2xl mb-6 transform hover:scale-105 transition-all duration-300 hover:shadow-blue-500/20"
loading="eager"
/>

                
                <p className="text-lg text-blue-400 font-semibold tracking-wider mb-2">{t('hero.subheading')}</p>
                <h1 className="text-5xl md:text-7xl font-extrabold mb-4">{t('hero.heading')}</h1>
                <p className="text-lg text-gray-300 max-w-2xl mb-8">{t('hero.description')}</p>
                
                <div className="flex flex-wrap justify-center items-center gap-4 mb-8">
                    <a href="https://calendly.com/gerardoleiserson" target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white font-bold py-3 px-8 rounded-full hover:bg-blue-700 transition-colors text-lg shadow-lg">
                        {t('hero.ctaAppointment')}
                    </a>
                    <button onClick={scrollToSell} className="bg-green-600 text-white font-bold py-3 px-8 rounded-full hover:bg-green-700 transition-colors text-lg shadow-lg">
                        {t('hero.ctaSell')}
                    </button>
                     <button onClick={openServicesModal} className="bg-purple-600 text-white font-bold py-3 px-8 rounded-full hover:bg-purple-700 transition-colors text-lg shadow-lg">
                        {t('hero.ctaServices')}
                    </button>
                    <a href="https://wa.me/5491163601511" target="_blank" rel="noopener noreferrer" className="bg-gray-700 text-white font-bold py-3 px-8 rounded-full hover:bg-gray-600 transition-colors text-lg shadow-lg">
                        {t('hero.ctaWhatsapp')}
                    </a>
                </div>
